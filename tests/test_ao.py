...
from agent_orchestrator.ao import main as ao_main
...
class TestSetup:
    def test_setup_passes_when_claude_exists(self, ao_env):
        runner, _ = ao_env
        with patch("agent_orchestrator.ao._check_claude", return_value=True):
    ...
    def test_setup_fails_when_claude_missing(self, ao_env):
        runner, _ = ao_env
        with patch("agent_orchestrator.ao._check_claude", return_value=False):
            result = runner.invoke(ao_main, ["setup"])
...
class TestStart:
    ...
    def test_creates_agent_orchestrator_dir(self, ao_env):
        runner, project_dir = ao_env
        with patch("agent_orchestrator.ao._check_claude", return_value=True), \
             patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(returncode=0)
    ...
    def test_claude_md_has_db_path(self, ao_env):
        ...
        claude_md = os.path.join(project_dir, "CLAUDE.md")
        content = open(claude_md).read()
        assert ".agent-orchestrator/bus.db" in content
    ...
    def test_skips_duplicate_claude_md_section(self, ao_env):
        ...
        claude_md = os.path.join(project_dir, "CLAUDE.md")
        content = open(claude_md).read()
        assert content.count("## Agent Orchestrator") == 1
